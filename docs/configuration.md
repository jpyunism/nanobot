...
| `NANOBOT_SKIP_WIZARD` | unset | Deprecated. The installer no longer launches the WebUI or wizard automatically, so this variable has no effect. Run `nanobot webui` or `nanobot onboard --wizard` manually after install. |
