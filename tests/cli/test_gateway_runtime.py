...
from nanobot.cli.commands import _close_gateway_runtime
...
async def test_finished_runtime_tasks_gather_is_retrieved() -> None:
    ...
    await runtime_tasks  # deterministically finish the gather before teardown
...
async def test_cancelled_runtime_tasks_gather_does_not_raise() -> None:
    ...
    for _ in range(3):
        await asyncio.sleep(0)  # let the cancellation propagate so it is done
